...
//定时器的中断服务
void Irq_my_timer(unsigned long dev)
{
    /* code */
    if(gpio_get_value(chdev.gpio))
    {
        led_on();
    }
    else
...
static int dev_open(struct inode *inode,struct file *filp)
{
    ...
    //开启定时器
    mod_timer(&tim,jiffies + msecs_to_jiffies(1000));//T = 2s
...
static int __init dev_init(void)//驱动入口
{
    ...
    //配置GPIO
    ret = gpio_request(chdev.gpio,"pin test");
    if(ret < 0)
    {
        printk("gpio request failed!\r\n");
        return EFAULT;
    }
    ret = gpio_direction_output(chdev.gpio,1);//off led 
    if(ret < 0)
    {
    ...
    //定时器初始化
    init_timer(&tim);
    tim.function = Irq_my_timer;
    tim.data     = (unsigned long)&chdev;
    //在open里开起定时器
