...
static ssize_t dev_read(struct file *filp,char __user *buf,size_t cnt,loff_t *offt)
{
    ...
    {
        if(gpio_get_value(chdev.gpio[0]) == 0)//没有按键按下
        {
            return -EAGAIN;
        }
    }
...
unsigned int dev_poll(struct file *filp,struct poll_table_struct *wait)
{
    ...
    /* codec */
    if(gpio_get_value(chdev.gpio[0]) == 1)//按键按下->等待事件
    {
        mask = POLLIN | POLLRDNORM;//等价的
        printk("poll reseponsed!\r\n");
    }
    //直到按键按下才返回有用的值
    return mask;
    //轮询返回事件可查询后，应用层再调用read进行再一次判断，读取，操作
    //这样避免了反复read设备文件，驱动层反复调用read函数
    //只需要在poll中轮询等待即可
}
